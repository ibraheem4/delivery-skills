...
Tell the user:
> Open **Keychain Access** → search "Developer ID Application" → right-click the certificate → **Export Items...** → save as `acme-signing.p12` on Desktop → set a password.
>
...
# Certificate (base64)
base64 -i ~/Desktop/acme-signing.p12 | gh secret set APPLE_CERTIFICATE -R $REPO
...
Remind the user:
> Delete `~/Desktop/acme-signing.p12` — the certificate is now in GitHub Secrets and shouldn't stay on disk unencrypted.
