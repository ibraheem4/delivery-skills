...
|-----------|-----------|-------------------|
| `marketing` | `acme-marketing` | `acme-marketing-dev` / `acme-marketing` |
| `validate` | `acme-validate` | `acme-validate-dev` / `acme-validate` |
| `mcp-server` | `acme-mcp-server` | `acme-mcp-server-dev` / `acme-mcp-server-prod` |
| `platform-api` | `acme-platform-api` | `acme-platform-api-dev` / `acme-platform-api` |
| `studio` | `acme-studio` | N/A (desktop app, uses GitHub Actions) |
...
```
gcloud builds list --limit 5 --region us-central1 --filter substitutions.REPO_NAME:acme-{service} --format table(id,status,startTime,duration,logUrl)
```
