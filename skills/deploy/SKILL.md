...
7. **Infrastructure sync reminder**: If `cloudbuild.yaml` was modified, remind to update Terraform in `acme-infrastructure/`.
...
| `ERROR: (gcloud.builds.list) PERMISSION_DENIED` | Missing GCP auth | Run `gcloud auth login` and `gcloud config set project acme-prod` |
