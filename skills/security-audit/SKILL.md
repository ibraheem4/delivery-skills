...
Systematically review branch protections, Dependabot, code scanning, secret scanning, and push protection across all Acme repositories. Target: 100% coverage.
...
```bash
gh repo list acme --limit 50 --json name,isArchived,visibility,defaultBranchRef \
...
gh api repos/acme/{repo}/branches/{branch}/protection --jq '{
...
```bash
gh api repos/acme/{repo}/branches/{branch}/protection --method PUT --input - <<EOF
{
...
gh api repos/acme/{repo}/vulnerability-alerts --method GET 2>&1
# 204 = enabled, 404 = disabled

# Enable if disabled (--fix mode)
gh api repos/acme/{repo}/vulnerability-alerts --method PUT 2>&1

# Check Dependabot security updates (auto-fix PRs)
gh api repos/acme/{repo}/automated-security-fixes --method GET 2>&1
# 200 with enabled:true = active

# Enable security updates (--fix mode)
gh api repos/acme/{repo}/automated-security-fixes --method PUT

# Check if dependabot.yml exists
gh api repos/acme/{repo}/contents/.github/dependabot.yml 2>&1
```
...
gh api repos/acme/{repo}/code-scanning/alerts --jq 'length' 2>&1
# If 403/404: not enabled

# Check for CodeQL workflow
gh api repos/acme/{repo}/contents/.github/workflows --jq '.[].name' 2>&1 | grep -i codeql
```
...
gh api repos/acme/{repo} --jq '.security_and_analysis.code_security.status'

# Enable code_security (required for private repos)
gh api repos/acme/{repo} --method PATCH --input - <<EOF
{
...
gh api repos/acme/{repo}/languages --jq 'keys'
...
gh api repos/acme/{repo}/code-scanning/default-setup --method PATCH \
...
```
Security Audit Report — Acme Organization
============================================

Branch Protection:
  ✅ acme-studio     dev: protected  main: protected
  ✅ acme-dev        dev: protected  main: protected
  ❌ acme-utils      main: NOT PROTECTED
  ...
