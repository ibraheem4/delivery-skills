...
- `acme-validate/` → `make validate-validate`
- `acme-studio/` → `make validate-studio`
- `acme-marketing/` → `make validate-marketing`
- `acme-platform-api/` → `make validate-platform-api`
- `acme-mcp-server/` → `make validate-mcp-server`
- `packages/` → `make validate-packages`
- Root-level only (skills, docs, config) → skip validation

Run from the `acme-dev` root. If multiple services changed, run each.
