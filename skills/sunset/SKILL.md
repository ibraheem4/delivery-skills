---
name: sunset
description: "Sunset a service, submodule, or repo: remove submodule from acme-dev, disable GCP infrastructure via Terraform, archive the GitHub repo, and clean up local directories. Use when user says 'sunset X', 'remove X', 'archive X', or 'decommission X'."
argument-hint: "<service-or-repo-name> [--dry-run] [--skip-terraform] [--skip-archive]"
---

# Sunset: Decommission a Service or Repo

You are running the `/sunset` workflow. This systematically removes a service/repo from the Acme ecosystem.

**Arguments:**
- `$ARGUMENTS` — the service or repo name (e.g., `acme-validate`, `platform-api`)
...
2. Check if it exists as:
   ...
   - A local directory in `acme-dev/`
   - A GCP service in `acme-infrastructure/services.tf` or `main.tf`
   - A GitHub repo under `acme/`
...
In our standard Acme Terraform modules, Cloud Run v2 services are typically created with `deletion_protection = true`. When that flag is enabled you CANNOT destroy the service resource in a single `terraform apply`.
...
1. Commit Terraform changes inside the `acme-infrastructure` submodule
2. Push the submodule: `cd acme-infrastructure && git push`
...
1. Use `gh repo archive acme/<repo-name> --yes` to archive the repo
